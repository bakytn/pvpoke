...
- Expected output: one `rankings-<cp>.json` per category — `overall`, `leads`, `closers`, `switches`, `chargers`, `attackers`, `consistency` (defenders/beaminess if the config adds them). Verify each file is non-trivial and that no banned species appears in it.
