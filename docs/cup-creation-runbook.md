...
- Rankings are precomputed files under `src/data/rankings/<cup>/<category>/rankings-<cp>.json` (categories: `overall`, `leads`, `closers`, `switches`, `chargers`, `attackers`, `consistency`, ...). A new cup with no rankings files simply shows no rankings — pool/filters still work for battles and team builder. Generating rankings is a separate, explicitly-requested step (Node ranker CLI per `src/scripts/*`).
